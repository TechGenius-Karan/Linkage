...
│   ├── vite.config.ts                # base: '/'  <- Netlify serves the site at its domain root
│   ├── netlify.toml                  # build/publish dirs, headers  (docs/deployment.md)
│   ├── tailwind.config.ts
...
└── .github/workflows/
    └── test.yml                      # pytest + vitest on every PR
                                       # (deploy has no workflow of its own — Netlify's
                                       #  own git integration builds + deploys web/ on
                                       #  push to main, per web/netlify.toml)
```
...
- [x] `vite.config.ts` — set `base: '/'`; Netlify serves the site at its domain root, not a project subpath.
...
**GitHub is already connected:** `origin → https://github.com/TechGenius-Karan/Linkage.git`, with `gh` authenticated as `TechGenius-Karan` (scopes: `repo`, `workflow`, `read:org`, `gist`).

**Host: Netlify.** *(Revised 2026-09-13 — was GitHub Pages; see the decision log below.)* Free, fully static-friendly, and its native git integration builds and deploys on every push without a bespoke deploy workflow — connecting the repo in the Netlify dashboard and committing `web/netlify.toml` is the whole setup. Two things tipped it over GitHub Pages specifically: Netlify's build config can set response headers and Pages cannot (closes the `manifest.json` cache-staleness question outright rather than accepting it as residual risk), and moving from `<site>.netlify.app` to a bought domain later is no harder here than on Render, which was the actual open question — at that point defaulting to the platform most of this project's other work already lives on is the tiebreaker. Full audit and migration plan in [`docs/deployment.md`](docs/deployment.md).

```
.github/workflows/test.yml     on: pull_request   -> pytest + vitest + tsc --noEmit
web/netlify.toml                on: push to main   -> Netlify's own build -> deploy + CDN
```

There is no `deploy.yml`. Netlify's git integration *is* the deploy step once the site is connected — it also builds a deploy preview for every PR at no extra cost, which the GitHub Pages plan would have needed a second workflow to get.

**Deployment gotchas, all of which bite silently:**

- `vite.config.ts` sets `base: '/'` — Netlify serves a site at its domain root, never a project subpath, so there is no subpath to misconfigure the way a GitHub Pages project site required (former Risk #7).
- All puzzle fetches must still go through `import.meta.env.BASE_URL` rather than a hardcoded `/puzzles/...` — not because Netlify needs it today, but so the one file that would ever have to change is `vite.config.ts`, not every call site.
- `web/public/puzzles/**` is **committed**, not generated in CI or by Netlify's build. The engine needs a 1.2 GB dataset; nothing in the deploy path may touch it. Regenerating puzzles is a deliberate local action followed by a commit. CI still runs the golden test — on `engine/fixtures/verification-subgraph.json`, not on the graph (§7.10).
- `engine/fixtures/**` is committed but **must never be copied into the published output**. It is a plaintext answer key. `netlify.toml`'s `base = "web"` already keeps Netlify's build from seeing anything outside `web/`, so this is structural rather than something to remember.
- `.gitignore` must cover `data/raw/`, `data/*.gpickle`, `node_modules/`, `dist/`, `engine/candidates.json`, `engine/approved.json`.

**Branching:** `main` is deployable at all times. Feature branches per phase (`phase-1-data-engine`, …), squash-merged via PR so `test.yml` gates every change, and Netlify's deploy preview on the PR itself gives a visual check before merge.
...
3. Commit and push. Netlify redeploys automatically; the CDN cache expires within minutes to hours, well inside the same day.
...
| 7 | ~~**GH Pages `base` path misconfigured**~~ | ~~High~~ **Moot under Netlify** | — | A Netlify site serves at its domain root, so there is no project-subpath `base` to get wrong the way a GitHub Pages project site required — `base: '/'` is now just Vite's default (§12). Kept here rather than deleted: the underlying lesson — a bundler path setting invisible in local dev, live only on the real host — is exactly why `docs/deployment.md` still calls for a manual check on the first real deploy rather than trusting a green build. |
...
| A bundler path setting is invisible in local dev, live only on the real host | `base: '/'` (Netlify roots the site — no subpath) + `import.meta.env.BASE_URL` kept in the fetch path regardless | §12 |
...
| Host | **Netlify** *(revised 2026-09-13, was GitHub Pages)* | GitHub Pages, Vercel, Render | Fully static client either way. Netlify's build config controls response headers, which Pages does not — closes the `manifest.json` cache-staleness question rather than accepting it as residual risk. Migrating `<site>.netlify.app` to a bought domain is no harder here than on Render, which was the actual deciding question once Pages was ruled out; Netlify won the tiebreak as the platform most of this project's other work already lives on. |
